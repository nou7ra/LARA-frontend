    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl">
        ...
        <div className="mb-4 space-y-2">
          ...
          {options.map((opt, i) => (
            <div key={i} className="flex items-center gap-3">
              <input type="radio" name="correct" checked={correctAnswer === opt && opt !== ""}
                onChange={() => setCorrectAnswer(opt)} className="accent-orange-500 w-4 h-4 flex-shrink-0" />
        ...
        <div className="flex gap-3 justify-end mt-4">
          <button onClick={onClose} className="px-5 py-2 rounded-full border border-gray-300 text-gray-600 text-sm hover:bg-gray-50">Cancel</button>
          <button onClick={handleSave} className="px-5 py-2 rounded-full bg-orange-500 text-white text-sm font-semibold hover:bg-orange-600">Save Question</button>
        </div>
      </div>
    </div>
  );
...
function FinalQuizContent() {
  ...
  const handleAddQuestion = () => { setEditingQuestion(null); setShowModal(true); };

  const handleEditQuestion = (id: number) => {
    const q = questions.find(q => q.id === id);
    if (q) { setEditingQuestion(q); setShowModal(true); }
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(to bottom, #FFD9B8, #FFE6C5)" }}>
      <FinalQuizNavbar />
      {showModal && (
        <QuestionModal question={editingQuestion} onSave={handleModalSave}
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        ...
        <QuizDetails date={date} onDateChange={setDate} duration={duration}
          onDurationChange={setDuration} durations={durationOptions} />
        <div className="space-y-4">
          {questions.map((q, index) => (
            <div key={q.id} className="bg-white rounded-2xl p-5 shadow-md border-l-4 border-orange-400">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => handleEditQuestion(q.id)}
                    className="w-9 h-9 rounded-lg border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition">✏️</button>
              ...
              <div className="space-y-2">
                {q.options.map((opt, i) => (
                  <div key={i} className={`px-4 py-2.5 rounded-xl text-sm border transition-all ${
                    opt === q.correctAnswer ? "bg-green-50 border-green-300 text-green-700 font-semibold" : "bg-gray-50 border-gray-200 text-gray-700"
                  }`}>
        ...
        <QuizActions onCancel={handleCancel} onSave={saving ? () => {} : handleSave} />
