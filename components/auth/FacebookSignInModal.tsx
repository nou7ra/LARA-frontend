...
interface FacebookSignInModalProps {
  isOpen: boolean;
  onClose: () => void;
  onContinue: (accountName: string) => void;
}

export default function FacebookSignInModal({ isOpen, onClose, onContinue }: FacebookSignInModalProps) {
  const [selectedAccount, setSelectedAccount] = useState<string | null>("Nourhan Hessen");
  ...
  return (
    ...
    >
      ...
      >
        {/* Header with Facebook Logo and Close Button */}
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-[#1877f2] flex items-center justify-center shadow-md">
              <FaFacebook className="text-xl text-white" />
            </div>
            <span className="text-base font-semibold text-gray-800">Facebook</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-8 py-6">
          {/* Title */}
          <h2 className="text-2xl font-normal text-gray-800 mb-2">Sign in with Facebook</h2>
          <p className="text-sm text-gray-600 mb-6">
            Continue to <span className="text-[#1877f2] font-medium">LARA</span>
          </p>

          {/* Selected Account */}
          <div
            className={`border-2 rounded-lg p-4 mb-3 cursor-pointer transition-all duration-300 ${
              selectedAccount === "Nourhan Hessen"
                ? "border-[#1877f2] bg-blue-50/30 shadow-md"
                : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"
            }`}
            onClick={() => setSelectedAccount("Nourhan Hessen")}
          >
            <div className="flex items-center gap-3">
              ...
              {selectedAccount === "Nourhan Hessen" && (
                <div className="flex-shrink-0">
                  <div className="w-6 h-6 rounded-full bg-[#1877f2] flex items-center justify-center transform scale-100 transition-transform duration-300">
          ...
          <div className="mb-6">
            <label className="flex items-start gap-3 cursor-pointer group">
              ...
              <span className="text-xs text-gray-600 leading-relaxed select-none">
                I agree to the{" "}
                <a href="#" className="text-[#1877f2] hover:underline" onClick={(e) => e.preventDefault()}>
                  Terms of Service
                </a>{" "}
                and{" "}
                <a href="#" className="text-[#1877f2] hover:underline" onClick={(e) => e.preventDefault()}>
            ...
            <button
              ...
              className={`px-8 py-2.5 text-sm font-medium text-white rounded-md transition-all duration-300 ${
                selectedAccount && agreedToTerms
                  ? "bg-[#1877f2] hover:bg-[#0c63d4] hover:shadow-lg hover:scale-105 active:scale-95"
